...
function resolveExecaCommand() {
  if (typeof execa.command === "function") return execa.command.bind(execa);
  if (typeof execa.execaCommand === "function") return execa.execaCommand.bind(execa);
  if (typeof execa === "function") {
    return (cmd, options) => execa(cmd, { ...options, shell: options.shell || true });
  }
  throw new Error("No compatible execa command runner found.");
}

async function run(command, cwd) {
  const shell = process.platform === "win32" ? resolveShell() : process.env.SHELL || true;
  const execaCommand = resolveExecaCommand();
  await execaCommand(command, { stdio: "inherit", cwd, shell });
}
